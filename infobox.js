...
class BigInfoBox extends InfoBox {
	...
	update() {
		...
		if (this.stage == 4 && this.game.spaceDown && vtsCalibrated) {
			this.stage = 5;
		}
	...
	draw(ctx) {
		...
		if (!vtsCalibrated) {
			this.animation.drawFrame(this.game.clockTick, ctx, this.game.liveCamera.x + this.game.liveCamera.width / 2 + this.animation.offsetX + 140,
				this.game.liveCamera.y + this.game.liveCamera.height / 2 + this.animation.offsetY + 160 + 32, 1);
		}
